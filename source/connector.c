...
{
		[ECU][SPEED_SETPOINT].msg.id = 0x181, 	[ECU][SPEED_SETPOINT].msg.length = 3, 	[ECU][SPEED_SETPOINT].period = 1,
		[ECU][TORQUE_SETPOINT].msg.id = 0x181,	[ECU][TORQUE_SETPOINT].msg.length = 3, 	[ECU][TORQUE_SETPOINT].period = 1,
		[ECU][ECU_STATUS].msg.id = 0x3D0, 		[ECU][ECU_STATUS].msg.length = 1, 		[ECU][ECU_STATUS].period = 100,
		[ECU][CONTROL].msg.id = 0x100, 			[ECU][CONTROL].msg.length = 1, 			[ECU][CONTROL].period = NO_REP ,
		[ECU][ECU_ERROR].msg.id = 0x101, 		[ECU][ECU_ERROR].msg.length = 1, 		[ECU][ECU_ERROR].period = NO_REP,
		[ECU][ECU_REQUEST].msg.id = 0x111, 		[ECU][ECU_REQUEST].msg.length = 1, 		[ECU][ECU_REQUEST].period = NO_REP,
		[ECU][BMS_CONTACTOR].msg.id = 0x331,	[ECU][BMS_CONTACTOR].msg.length = 1, 	[ECU][BMS_CONTACTOR].period = NO_REP,
...
static THD_FUNCTION(CANSend, arg) {
    ...
    while(1){
    	...
    	if(treat_ECU && output)
    		send_one_type(ECU, period_count);
    ...
    while (1) {
		...
		if(treat_ECU)
			for (i = 0; i<ECU_INTEREST_LIST_SIZE; i++)
				if(rxf.SID == ECU_interest_list[i])
				{
#if MY_TYPE != ECU
					if(!do_readout)
						send_to_PC(input.msg) ;
...
#if (MY_TYPE != ECU)
				treat_ECU = data[0]&SET_TREAT_ECU;
#endif
#if MY_TYPE != MOTOR
